...
В IOST смарт-контракты будут закодированы в JavaScript `class`. При его использовании вам нужно явно `export` класс.

#### Структура смарт-контракта

Класс смарт-контракта должен включать в себя `Init` и `Constructor` функции.
...
Класс BlockChain предоставляет все методы для вызова системы и помогает пользователю вызывать официальные API-интерфейсы, включая, помимо прочего, передачу, проводку денег, вызов других контрактов и поиск блока или транзакции.
...
В настоящее время IOST поддерживает только большие числа типа `Int64`. Пожалуйста, воздержитесь от использования других типов чисел.
