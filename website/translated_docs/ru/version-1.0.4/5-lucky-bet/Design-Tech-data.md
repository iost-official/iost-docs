...
Удачная ставка - азартная игра, работающая на смарт-контракте IOST. Он был разработан, чтобы продемонстрировать написание и развертывание смарт-контракта.
...
В возвращаемых значениях, `s` является префиксом типа `String`. `66` это значение, которое мы пишем после `put`. Последний результат (`result65`) сохраняется в строковой переменной. Получив доступ к хранилищу смарт-контракта, мы можем получить статус и результаты контракта для запуска и отображения бэкэнда.
